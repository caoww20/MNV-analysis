...
with open(input) as f, open(output_file+'tar_ref.seq','w') as result1,open(output_file+'tar_SNV.seq','w') as result2, open(output_file+'tar_MNV.seq','w') as result3, open(output_file+'miranda_ref.seq','w') as result4,open(output_file+'miranda_SNV.seq','w') as result5, open(output_file+'miranda_MNV.seq','w') as result6:
    for i in f:
        ...
        result1.write('|'.join([line[5],mnvid,ncRNA_ID])+'\t9606\t'+ref.replace("T", "U")+'\n')
        result4.write('>'+'|'.join([line[5],mnvid,ncRNA_ID,ncRNA_strand])+'\n')
        ...
        for ii in range(len(mnv_pos)):
            ...
            result2.write('|'.join([rsids[ii],mnvid,ncRNA_ID])+'\t9606\t'+alt_snv.replace("T", "U")+'\n')
            result5.write('>'+'|'.join([rsids[ii],mnvid,ncRNA_ID,ncRNA_strand])+'\n')
        ...
        result3.write('|'.join([line[5],mnvid,ncRNA_ID])+'\t9606\t'+alt_mnv.replace("T", "U")+'\n')
        result6.write('>'+'|'.join([line[5],mnvid,ncRNA_ID,ncRNA_strand])+'\n')
