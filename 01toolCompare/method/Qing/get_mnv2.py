...
os.environ["HAIL_DRIVER_MEMORY"] = "10g"  # 设置Driver的内存占用为10GB
os.environ["HAIL_EXECUTOR_MEMORY"] = "10g"  # 设置Executor的内存占用为10GB

vcf_url=sys.argv[1]
# vcf_url="/data/jinww/mnv/analyse/part1/data/cost/test2.vcf"
vcf = hl.import_vcf(vcf_url, call_fields=["GT"]) #change the call_fields according to the hail documentation
vcf = hl.split_multi_hts(vcf)
vcf.write(vcf_url + ".mt", overwrite=True)
mt = hl.read_matrix_table(vcf_url + ".mt")
...
mt = hl.window_by_locus(mt, 10) #partition in window -- we only care within codon reading frame, so the max distance is set to be 2
...
et_het.write(vcf_url + "et_het.ht", overwrite=True) #and we are going to save them
et_hethom = et.filter(et.hethom)
et_hethom.write(vcf_url + "et_hethom.ht", overwrite=True)
et_hom = et.filter(et.hom)
et_hom.write(vcf_url + "et_hom.ht", overwrite=True)
...
#write
# comb.write(vcf_url + "_mnv_combined.ht", overwrite=True)
comb.export(vcf_url[0:-4] + ".mnv")
