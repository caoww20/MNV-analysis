...
for(cancer in cancer.list){
  ...
  raw.data <- fread(paste0("/home/luohh/MNVAnalysis/01.GWAS/02.Cancer/01.Data/02.PhenoData/",cancer,"/clinic.geno.02.tsv"),header = T,stringsAsFactors = F,data.table = F)
  ...
  for(mnv in unique(raw.data$MNVID)){
    ...
    if (length(unique(data.mnv$genotype)) == 2){
      if (all(unique(data.mnv$genotype) == c(0,2))){
        geno0.num <- nrow(data.mnv[data.mnv$genotype=="0",])
        geno1.num <- nrow(data.mnv[data.mnv$genotype=="2",])
        ...
        ggsave(plt$plot,filename = paste0("/home/luohh/MNVAnalysis/01.GWAS/02.Cancer/04.Plot/01.SurvivalPlot/10years/02/",cancer.name,"-",mnv,".pdf"),width = 6,height = 5)
...
colnames(results.matrix) <- c("Cancer","MNVID","KM_P")
write.table(results.matrix,"/home/luohh/MNVAnalysis/01.GWAS/02.Cancer/04.Plot/01.SurvivalPlot/10years/02/11Cancers.survival.p.txt",quote = F,row.names = F,col.names = T)
```
  ...
  for(mnv in unique(raw.data$MNVID)){
    ...
    if (length(unique(data.mnv$genotype)) == 2){
      if (all(unique(data.mnv$genotype) == c(0,2))){
        ...
        ggsave(plt$plot,filename = paste0("/home/luohh/MNVAnalysis/01.GWAS/02.Cancer/04.Plot/01.SurvivalPlot/allTime/02/",cancer.name,"-",mnv,".pdf"),width = 6,height = 5)
...
colnames(results.matrix) <- c("Cancer","MNVID","KM_P")
write.table(results.matrix,"/home/luohh/MNVAnalysis/01.GWAS/02.Cancer/04.Plot/01.SurvivalPlot/allTime/02/11Cancers.survival.p.txt",quote = F,row.names = F,col.names = T)
```
