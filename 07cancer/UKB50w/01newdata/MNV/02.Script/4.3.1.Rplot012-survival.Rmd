...
for(cancer in cancer.list){
  ...
  for(mnv in unique(raw.data$MNVID)){
    ...
    if (length(unique(data.mnv$genotype)) == 3){
      ...
      ggsave(plt$plot,filename = paste0("/home/luohh/MNVAnalysis/01.GWAS/02.Cancer/04.Plot/01.SurvivalPlot/10years/012/",cancer.name,"-",mnv,".pdf"),width = 6,height = 5)
...
colnames(results.matrix) <- c("Cancer","MNVID","KM_P")
write.table(results.matrix,"/home/luohh/MNVAnalysis/01.GWAS/02.Cancer/04.Plot/01.SurvivalPlot/10years/012/11Cancers.survival.p.txt",quote = F,row.names = F,col.names = T)
```
...
for(cancer in cancer.list){
  ...
  for(mnv in unique(raw.data$MNVID)){
    ...
    if (length(unique(data.mnv$genotype)) == 3){
      ...
      ggsave(plt$plot,filename = paste0("/home/luohh/MNVAnalysis/01.GWAS/02.Cancer/04.Plot/01.SurvivalPlot/allTime/012/",cancer.name,"-",mnv,".pdf"),width = 6,height = 5)
...
colnames(results.matrix) <- c("Cancer","MNVID","KM_P")
write.table(results.matrix,"/home/luohh/MNVAnalysis/01.GWAS/02.Cancer/04.Plot/01.SurvivalPlot/allTime/012/11Cancers.survival.p.txt",quote = F,row.names = F,col.names = T)
```
