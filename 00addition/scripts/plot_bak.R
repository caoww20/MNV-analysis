...
mythem <- theme(
  ...
  axis.text.x = element_text(vjust = 1, size = 12, face = "bold", colour = 'black', angle = 45, hjust = 1),
  axis.title.x = element_text(vjust = 1, size = 12, face = "bold", color = 'black'),
...
ggplot(plot_data, aes(x = sum_snv_effects, y = diff)) +
  ...
  labs(
    x = "Sum of SNV effects (kcal/mol)",
    y = "Difference (MNV effects - sum of SNV effects)",
    color = "Difference (kcal/mol)"
  ) +
  theme_bw()+mythem
...
mythem <- theme(
  ...
  panel.border = element_rect(fill = NA, color = "black", size = 1, linetype = "solid"),
  text = element_text(size = 12, face = "bold"),
  axis.text.x = element_text(vjust = 1, size = 12, face = "bold", colour = 'black'),
  axis.title.x = element_text(vjust = 1, size = 12, face = "bold", color = 'black'),
  axis.text.y = element_text(hjust = 1, size = 12, face = "bold", color = 'black'),
  axis.title.y = element_text(vjust = 1, size = 12, face = "bold", color = 'black')
)
...
ggplot(plot_data, aes(x = type_label, y = number)) +
  ...
  labs(
    x = "MNV pattern",
    y = "number of adjacent 2-joint MNVs"
  )

ourdir <- '/home/caow/03mnv/analyse3/00addition/source_data'
write.csv(df_ratio %>% select(c(1:3)), file.path(ourdir, 'figureS9-ab.csv'), row.names = FALSE, quote = FALSE)
write.csv(plot_data %>% select(c(10,11)), file.path(ourdir, 'figureS12.csv'), row.names = FALSE, quote = FALSE)
write.csv(data_1000G, file.path(ourdir, 'figureS9-c.csv'), row.names = FALSE, quote = FALSE)
write.csv(data_GTEx, file.path(ourdir, 'figureS9-d.csv'), row.names = FALSE, quote = FALSE)
write.csv(plot_data, file.path(ourdir, 'figureS11-a1.csv'), row.names = FALSE, quote = FALSE)
write.csv(gc_data %>% select(c(19)), file.path(ourdir, 'figureS11-a2.csv'), row.names = FALSE, quote = FALSE)
write.csv(correlation_data, file.path(ourdir, 'figureS11-b.csv'), row.names = FALSE, quote = FALSE)

write.csv(final_plot_data , file.path(ourdir, 'figureS11-c.csv'), row.names = FALSE, quote = FALSE)
